...
class test_tree_node:
    ...
    def test_tree_has_id3_info(self, authors_guide_to_murder__flat_mp3: Audiobook, setup_teardown):
        ...
        for x in [i.children, i.files, i.dirs, i.this_and_siblings]:
            ...
            assert x.part_nums == [] or x.part_nums == [1, 2]
        ...
        assert i.children.all_path_nums == [[(n, 0)] for n in range(1, 39)]

        # Files
        assert i.files.start_nums == [n for n in range(2, 39)]
        assert i.files.all_path_nums == [[(n, 0)] for n in range(2, 39)]
        ...
        assert i.children_recursive.all_path_nums == [[(1, 0)]] + [[(n, 0)] for n in range(1, 39)]
        ...
        assert i.files_recursive.all_path_nums == [[(n, 0)] for n in range(1, 39)]
        ...
        assert i.dirs.all_path_nums == [[(1, 0)]]
        # There are no id3 tags for dirs

        # This and siblings
        assert i.this_and_siblings.start_nums == []
        assert i.this_and_siblings.all_path_nums == [[(3, 32)]]  # Dir has "_mp3" in it
