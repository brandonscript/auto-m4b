...
def load_test_fixture(
    ...
    *,
    ...
    if src.is_dir():
        ...
        for f in src_globs:
            ...
            if dst_exists and src_size == min(dst_size, max_file_size):
                if exclusive and is_audio_file(dst_f):
                    dst_f.touch()  # refresh mtime so dir_was_recently_modified works correctly
                continue
    ...
    book = Audiobook(dst)
    _ = book.orig_file_type  # pre-compute while files still exist in inbox
    yield book
...
def load_test_fixtures(
    *names: str,
    ...
    holds_filter = exclusive or match_filter is not None
    original_match_filter: str | None = cfg.MATCH_FILTER if holds_filter else None
    ...
    # Each load_test_fixture call consumes the generator (including teardown), so the
    # match filter is reset after every sub-fixture.  Re-apply it once here so the
    # test body sees the correct filter.
    if holds_filter:
        testutils.set_match_filter(match_filter)

    yield fixtures

    # Restore the filter to whatever it was before we started loading fixtures.
    if holds_filter:
        testutils.set_match_filter(original_match_filter or None)
...
@pytest.fixture(scope="function", autouse=False)
def fails__multipart_mp3():
    """A multi-part book that fails because it has Part X dirs, but can be fixed by flattening."""
    name = "fails__multipart_mp3"
    book = TEST_DIRS.inbox / name
    book.mkdir(parents=True, exist_ok=True)

    for part_num in range(1, 4):
        part_dir = book / f"Part {part_num}"
        part_dir.mkdir(parents=True, exist_ok=True)
        for track_num in range(1, 3):
            idx = (part_num - 1) * 2 + track_num
            testutils.make_mock_file(part_dir / f"fails__multipart_mp3_{idx:02d}.mp3")
...
def reset_all(reset_match_filter, reset_failed):
    ...
    # Do NOT eagerly recreate InboxState here — fixtures load their files after
...
def requires_empty_inbox():
    ...
    # Remove any files/dirs created by the test that weren't in the original inbox
    if backup_inbox.exists():
        backup_names = {f.name for f in backup_inbox.glob("*")}
        for f in list(TEST_DIRS.inbox.glob("*")):
            if f.name not in backup_names:
                testutils.rm(f)
