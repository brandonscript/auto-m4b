...
from src.lib.typing import ENV_DIRS, OnComplete
...
class testutils:
    ...
    def flatten_book(cls, book: Audiobook, delay: int = 0):
        ...
        # Touch all moved files to update mtime so dir_was_recently_modified detects
        # the change (shutil.move preserves original mtime, which would be too old).
        for f in book.inbox_dir.rglob("*"):
            if f.is_file():
                f.touch()
    ...
    def force_inbox_hash_change(cls, *, delay: int = 0, age: float = 0.5):
        ...
        inbox.stale = True
        # Reset _last_banner_lc so the next process_inbox() run treats itself
        # as a fresh outer iteration and prints a new banner header if needed.
        # Do NOT reset banner_printed here — the test may have set it True to
        # simulate a "previous session" banner, and clearing it would cause a
        # spurious reprint that the test is explicitly checking against.
        inbox._last_banner_lc = -1
    ...
    @classmethod
    def backdate_inbox_files(cls, seconds: float | None = None):
        """Set all inbox audio file mtimes to the past so dir_was_recently_modified
        immediately returns False.  Replaces time.sleep() calls that previously had
        to wait for files to age past cfg.WAIT_TIME.
        """
        from src.lib.config import cfg
        from src.lib.fs_utils import is_audio_file

        if seconds is None:
            seconds = cfg.WAIT_TIME * 3 + 1

        old_mtime = time.time() - seconds
        old_atime = old_mtime
        inbox_dir = cfg.inbox_dir
        for f in inbox_dir.rglob("*"):
            if f.is_file() and is_audio_file(f):
                os.utime(f, (old_atime, old_mtime))
    ...
    def get_all_processed_books(cls, s: str, *, root_dir: Path = TEST_DIRS.inbox) -> list[str]:
        _AUDIO_EXTS = {".m4b", ".mp3", ".m4a", ".flac", ".ogg", ".aac", ".wav"}
        lines = s.splitlines()
        books = []
        for i, line in enumerate(lines):
            if line.startswith("- Source: "):
                ...
                # Normalize standalone audio-file sources to their parent directory key
                # e.g. "MyBook/MyBook.m4b" → "MyBook" to match the fixture's directory key
                key_path = Path(src)
                if key_path.suffix.lower() in _AUDIO_EXTS:
                    src = str(key_path.parent)
                books.append(src)
        return books
    ...
    def assert_processed_output(
        ...
        *exp_books: str | Path | Audiobook | BooksTree,
        ...
        def _book_key(b: "str | Path | Audiobook | BooksTree") -> str | None:
            """Extract an inbox-relative key for any kind of book reference.

            ``get_all_processed_books`` returns keys of the form
            ``<path-relative-to-inbox>``, so we normalise every expected-book
            reference to the same format so the comparison works regardless of
            whether the caller passed a raw Path, an Audiobook, or a BooksTree.
            """
            if isinstance(b, BooksTree):
                return b.key
            p = b.path if isinstance(b, Audiobook) else Path(b) if isinstance(b, str) else b
            try:
                return str(p.relative_to(TEST_DIRS.inbox))
            except ValueError:
                pass
        ...
        book_keys: list[str | None] = [_book_key(b) for b in exp_books]

        processed = cls.get_all_processed_books(out)
        exact_matches = [key in processed for key in book_keys]
        ...
        assert ok, f"Expected {len(book_keys)} to be converted: {books_list}\n\nGot {len(processed)}: {processed_list}"
    ...
    def assert_converted_book_and_collateral_exist(cls, book: Audiobook, quality: str):
        assert book.converted_dir.exists(), f"Converted dir missing: {book.converted_dir}"

        # The output m4b may be named after the book title (not the inbox folder) when
        # the folder is named after the author.  Find any .m4b in the converted dir.
        m4bs = list(book.converted_dir.rglob("*.m4b"))
        assert m4bs, f"No .m4b found in {book.converted_dir}"
        m4b = m4bs[0]
        assert m4b.stat().st_size > 0, f"Output .m4b is empty: {m4b}"

        # Old-style auto-m4b.*.log files must not be left behind
        log = book.converted_dir / f"auto-m4b.{book.basename}.log"
        assert not log.exists(), f"Stale log file left in converted dir: {log}"

        # Description file is named "{title or basename} [{quality}].txt".
        # Search for any .txt containing the quality stamp rather than hardcoding the stem.
        quality_stamp = f"[{quality}]"
        descs = [f for f in book.converted_dir.rglob("*.txt") if quality_stamp in f.name]
        assert descs, (
            f"No description .txt containing '{quality_stamp}' found in {book.converted_dir}"
        )
        assert descs[0].stat().st_size > 0, f"Description .txt is empty: {descs[0]}"
