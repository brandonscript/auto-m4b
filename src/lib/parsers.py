...
def parse_narrator(s: str, target: NameParserTarget, *, fallback: str | None = None, max_chars: int = 150) -> str:
    ...
    if target == "comment":
        # For comment targets (e.g. "Performed by Melody Muze as Feyre, Anthony Palmini as Rhysand..."),
        # apply the comment pattern directly to get only the PRIMARY narrator without splitting on all
        # commas in the cast list first. parse_names would otherwise join all performer names.
        m = narrator_comment_pattern.search(s)
        if m:
            raw = m.group("narrator").strip()
            # Strip "as <Character>" suffix, then take only the first name before any comma/semicolon
            primary = re.split(r"\s+as\s+|[,;]", raw, maxsplit=1)[0].strip()
            if primary:
                return primary
