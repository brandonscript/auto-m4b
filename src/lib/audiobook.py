...
class Audiobook(BaseModel):
    ...
    def orig_file_type(self):
        ...
        # For standalone files, derive type directly from the file extension.
        if self.tree.is_file():
            self._orig_file_type = to_audiobook_fmt(self.path.suffix) or "mp3"
            return self._orig_file_type
        # For directories, scan for the first audio file.  Skip scanning only when
        # this book *is* the inbox root itself (scanning would touch all books).
        if not (
            orig_file_type := (
                to_audiobook_fmt(f.suffix)
                if self.path != cfg.inbox_dir and (f := find_first_audio_file(self.path, ignore_errors=True))
                else None
            )
        ):
            return "mp3"
    ...
    def rescan(self):
        ...
        # Rebuild the tree from scratch, scoped to just this book's inbox dir.
        # Calling scan(allow_non_root=True) on the existing subtree re-globs from
        # the inbox root and incorrectly includes sibling books in _dirs, giving a
        # wrong (inflated) file count and corrupting the tree for subsequent books.
        new_tree = BooksTree(self.inbox_dir)
        self.tree = new_tree
        if item := InboxState().get(self.inbox_dir):
            item.tree = new_tree
