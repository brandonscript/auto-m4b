...
class BooksTree(BaseModel):
    ...
    _i: "TreeNodeSummary" = None  # type: ignore
    ...
    def key(self):
        ...
        if not (root := self.root):
            if self.path == cfg.inbox_dir:
                ...
                return "__tmp__"
            # Return the inbox-relative path for any path under the inbox.
            # Avoid is_dir() which checks the filesystem and returns False after
            # the book has been archived (moved off disk), causing the key to fall
            # back to self.name and breaking converted_dir for series sub-books.
            from src.lib.fs_utils import try_relative_to
            if rel := try_relative_to(self.path, cfg.inbox_dir):
                return str(rel)
            return self.name

        # For standalone file book-roots that are inside their own same-named directory
        # (e.g. inbox/MyBook/MyBook.m4b), return just the filename so that converted_dir
        # stays at the right level. Files in series/multi-parent dirs must use the full
        # relative path so their keys are rooted correctly.
        # Non-book-root files (e.g. tracks inside a book dir) still return None below.
        if self.is_file() and self.is_book_root:
            # Only collapse to bare name when parent dir name matches the stem,
            # meaning the file is already inside its own dedicated directory.
    ...
    def determine_if_book_root(self):
        ...
        # Files that are children of a book-like directory are NOT book roots themselves.
        # They belong to their parent directory (e.g. after flattening a multi-part book,
        # individual mp3 files should NOT be treated as separate book roots).
        if self.is_file() and (p := self.parent) and p.has_any_structure(
            "flat", "multi_disc", "multi_part", "multi_parent", "flatish"
        ):
            self._is_book_root = False
            return self._is_book_root
