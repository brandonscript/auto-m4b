...
InboxItemStatus = Literal["new", "ok", "needs_retry", "failed", "gone", "processed"]
...
class InboxItem:
    ...
    def update_path(self, path: Path):
        self.tree.path = path
        # Clear cached file/dir status so is_file()/is_dir() re-evaluate against
        # the new path (e.g., after a standalone file is moved into its own folder,
        # the old _is_file=True cache must not carry over to the directory).
        self.tree._is_file = None
        self.tree._is_dir = None
        # Invalidate the @cached_property so it recomputes for the new path.
        self.tree.__dict__.pop("key", None)
    ...
    def set_failed(self, reason: str, last_updated: float | None = None):
        ...
        # Record when this item first failed so we can evict it after 30 days.
        # Don't overwrite an existing timestamp so repeated failures don't reset the clock.
        if self.failed_at is None:
            self.failed_at = last_updated or time.time()
    ...
    def set_processed(self):
        """Mark item as processed without removing it from disk.

        Used by ON_COMPLETE=test_do_nothing so the book stays in the inbox
        but is not re-converted in subsequent loops.  Unlike set_gone(), this
        status survives rescan without being reset to "new" (scan only resets
        "gone" items, not "processed" ones).
        """
        self._set("processed")
